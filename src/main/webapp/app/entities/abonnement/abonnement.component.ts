...
export class AbonnementComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.abonnementService
        ...
        .subscribe(
          (res: HttpResponse<IAbonnement[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
          () => this.onError()
        );
      return;
    }
  ...
  trackId(index: number, item: IAbonnement): string {
  ...
  protected onSuccess(data: IAbonnement[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
