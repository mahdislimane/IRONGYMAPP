...
/**
 * REST controller for managing {@link irongym.domain.Abonne}.
 */
@RestController
@RequestMapping("/api")
public class AbonneResource {
    ...
    /**
     * {@code PUT  /abonnes} : Updates an existing abonne.
     *
    ...
    public ResponseEntity<Abonne> updateAbonne(@RequestBody Abonne abonne) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, abonne.getId()))
            .body(result);
    }
    ...
    public ResponseEntity<Abonne> getAbonne(@PathVariable String id) {
    ...
    public ResponseEntity<Void> deleteAbonne(@PathVariable String id) {
    ...
    /**
     * {@code SEARCH  /_search/abonnes?query=:query} : search for the abonne corresponding
     * to the query.
     *
     * @param query the query of the abonne search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/abonnes")
    public ResponseEntity<List<Abonne>> searchAbonnes(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Abonnes for query {}", query);
        Page<Abonne> page = abonneService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
        }
}
